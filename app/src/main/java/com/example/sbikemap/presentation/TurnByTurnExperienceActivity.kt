...
class TurnByTurnExperienceActivity : AppCompatActivity() {
    ...
    private val overviewPadding: EdgeInsets by lazy {
        EdgeInsets(
            50.0 * pixelDensity, // Cũ: 140.0
            20.0 * pixelDensity, // Cũ: 40.0
            50.0 * pixelDensity, // Cũ: 120.0
            20.0 * pixelDensity  // Cũ: 40.0
        )
    }
    ...
    private val followingPadding: EdgeInsets by lazy {
        EdgeInsets(
            100.0 * pixelDensity, // Cũ: 180.0
            20.0 * pixelDensity,  // Cũ: 40.0
            100.0 * pixelDensity, // Cũ: 150.0
            20.0 * pixelDensity   // Cũ: 40.0
        )
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.mapView.mapboxMap.loadStyle(NavigationStyles.NAVIGATION_DAY_STYLE) { style ->
            ...
            // THÊM ĐOẠN NÀY ĐỂ TRÁNH LỖI MISSING IMAGE
            val drawable = androidx.core.content.ContextCompat.getDrawable(this, R.drawable.ic_navigation_puck) // Dùng luôn icon puck
            if (drawable != null) {
                // Nạp icon bóng và icon top để Mapbox không báo lỗi
                style.addImage("mapbox-location-shadow-icon", com.example.sbikemap.utils.bitmapFromDrawableRes(this, R.drawable.ic_navigation_puck)!!)
                style.addImage("mapbox-location-top-icon", com.example.sbikemap.utils.bitmapFromDrawableRes(this, R.drawable.ic_navigation_puck)!!)
            }
            // Kiểm tra xem có dữ liệu từ MapScreen gửi sang không, nếu có thì chạy luôn
...
//        replayOriginLocation()
    ...
    private fun findRoute(destination: Point) {
        ...
        if (originLocation == null) {
            Toast.makeText(this, "Chưa lấy được vị trí hiện tại của xe!", Toast.LENGTH_SHORT).show()
            return
        }
        ...
        mapboxNavigation.requestRoutes(
            ...
            object : NavigationRouterCallback {
                ...
                override fun onFailure(reasons: List<RouterFailure>, routeOptions: RouteOptions) {
                    Toast.makeText(this@TurnByTurnExperienceActivity, "Lỗi tìm đường: ${reasons.firstOrNull()?.message}", Toast.LENGTH_LONG).show()
                }
    ...
    // Hàm này kiểm tra xem có tọa độ gửi sang không
    private fun checkIntentForNavigation() {
        ...
        Toast.makeText(this, "Nhận tọa độ: $destLat, $destLon", Toast.LENGTH_LONG).show()
        if (destLon != 0.0 && destLat != 0.0) {
            val destination = Point.fromLngLat(destLon, destLat)

            // Nếu có điểm xuất phát gửi sang, ta cập nhật vị trí xe ngay lập tức
            if (originLon != 0.0 && originLat != 0.0) {
                val origin = Point.fromLngLat(originLon, originLat)
                // Cập nhật vị trí giả lập về chỗ của BẠN (thay vì San Francisco)
                updateReplayLocation(origin)
            }

            // Tự động tìm đường luôn
            findRoute(destination)
        } else {
            Toast.makeText(this, "Lỗi: Không có tọa độ điểm đến!", Toast.LENGTH_SHORT).show()
        }
    }

    // Hàm cập nhật vị trí xe về tọa độ bất kỳ
    private fun updateReplayLocation(point: Point) {
